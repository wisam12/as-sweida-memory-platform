...
export default function AdminPanel() {
    ...
    const fetchHeroes = async () => {
        setLoading(true);
        const res = await fetch(`${import.meta.env.VITE_API_URL}/api/pending-heroes`);
    ...
    const handleApprove = async (id) => {
        await fetch(`${import.meta.env.VITE_API_URL}/api/approve`, {
            method: "POST",
    ...
    const handleReject = async (id) => {
        await fetch(`${import.meta.env.VITE_API_URL}/api/reject/${id}`, { method: 'DELETE' });
        fetchHeroes();
    };
